...
struct Index: RouteCollection {
    ...
    func construction(req: Request) throws -> EventLoopFuture<HTML> {
        ...
        _ = Partners(img: "https://i0.wp.com/switchindiefix.com/wp-content/uploads/2018/09/cropped-logo-text-red.png", link: "http://switchindiefix.com")
        return Views.underConstruction(/*sif, */mwr).encodeResponse(for: req)
	}
    
    func index(req: Request) throws -> EventLoopFuture<HTML> {
        Manga.query(on: req.db).all().flatMap{ mangas in
            Views.index(mangas: mangas).encodeResponse(for: req)
        }
    }
